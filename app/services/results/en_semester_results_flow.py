...
from ...utils.results_utils import safe_decimal, get_grade_from_scale, is_fail_grade, get_grade_from_score_raw
import logging

logger = logging.getLogger(__name__)

def generate_en_semester_report(
    db, 
    student_id: int, 
    academic_id: int, 
    program_id: int, 
    ...
    """
    Generates the semester report for EN students.
    - Breakdown by Subject (aggregated from monthlies, simplified)
    ...
    for sid_subj, data in subj_agg.items():
        ...
        # Get Grade
        subj_grade_val, sub_kh_grade = get_grade_from_score_raw(avg, db, academic_id, grade_group_id, Decimal('100.0'))
