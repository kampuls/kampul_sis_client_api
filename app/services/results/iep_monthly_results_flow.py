...
def generate_iep_monthly_report(
    db, 
    student_id: int, 
    academic_id: int, 
    program_id: int, 
    ...
    """
    Generates the IEP monthly/input result report.
    ...
    if subject_ids:
        ...
        for sub_id in subject_ids:
            ...
            # Check if student has mark for this subject (for divisor adjustment)
            # keys are only present if DB row exists
            has_mark = sub_id in student_marks
            if has_mark:
                 present_subject_count += 1
            ...
            # Normalization / Deduction Logic (matching Kortra)
            adjusted_mark = raw_mark
            if rule and calc_m != Decimal('100'):
                if raw_mark < (full_m / Decimal('2')):
                    adjusted_mark = Decimal('0')
                else:
                    deduction_base = full_m * (calc_m / Decimal('100'))
            ...
            final_total += adjusted_mark
            student_subject_count += 1
            
            # Get Grade for Subject
            percentage_score = (raw_mark / full_m) * Decimal('100') if full_m > Decimal('0') else Decimal('0')
            sub_grade, sub_kh_grade = get_grade_from_score_raw(percentage_score, db, academic_id, grade_group_id, Decimal('100'))
            
            # Calculate Rank for Subject (SQL) - SCOPED TO CLASS
            # Compare raw marks in marks_input for this subject & marks_system within the same class
            sub_rank_q = text("""
                SELECT COUNT(*) + 1
                FROM marks_input mi
            ...
            details["items"].append({
                "type": "subject",
                "label": s_name,
                "score": None if is_missing else float(raw_mark),
                "average": None, 
        ...
        if student_subject_count > 0:
             ...
             # Kortra Divisor Logic
             expected_subjects_count = len(subject_ids)
             missing_subjects_count = max(0, expected_subjects_count - present_subject_count)
             
             # Adjust divisor based on missing subjects
             # divide_by is the multiplier (e.g. 5.0 for 5 subjects)
             adjusted_divisor = divide_by - Decimal(str(missing_subjects_count))
             if adjusted_divisor < Decimal('1'):
                 adjusted_divisor = Decimal('1')
             ...
             if mm_row:
                 ...
                 # Compare average in marks_monthly
                 my_avg = safe_decimal(mm_row[1], Decimal('0'))
                 
                 # Optimization: Filter by grade_id/class to rank within class
                 # Need to join learning/students? 
                 # Usually monthly rank is within class (grade_id + shift_id + academic_id + program_id)
                 # But marks_monthly doesn't have grade_id column directly usually, or does it?
                 # Let's check schema via query or inference. 
                 # KH Monthly flow used: JOIN learning l ... WHERE l.gradeid = :gid
                 # We should replicate that for accuracy.
                 ...
                 # Best-effort ranking against stored results of OTHERS
                 # This allows a student viewing their preview to see where they WOULD rank against currently saved results
                 scope_sql, scope_params = class_scope_rank_sql_suffix(
                     grade_type_id, branch_id
                 )
                 summ_rank_q = text(f"""
