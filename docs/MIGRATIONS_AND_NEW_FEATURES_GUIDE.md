...
2. It executes `template_school_db.sql` to create every table and load the presets of **one branch ("Main Campus") and one academic year**: roles, permissions, departments, positions, programs, grades, subjects, marks systems, grade scales, holidays and similar reference data. The template contains no users, students, invoices, contacts, bot tokens, GPS locations, uploaded files, fees or forms.
3. It creates the Administrator user (`ADM-001`, super admin and app admin) with the password generated by the platform, hashed with bcrypt. It refuses to run against a database that already has tables.
...
#### Step 2: Regenerate `template_school_db.sql` for Future Schools
Do not edit the template by hand. It is generated from a known-good local school database (currently `working test`):
1. Apply your change to that local database (new table/column, or new preset rows).
2. If the new table holds preset data every school should get, add it to the lists at the top of `scripts/build_school_template.py` (`COPY_ALL` for global presets, `SCOPED` for per-branch/per-academic-year presets). Tables not listed there stay empty.
3. Run `python scripts/build_school_template.py --source "working test"` and commit the regenerated template.
